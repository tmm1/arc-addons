...
if [ "${1}" = "modules" ]; then
  ...
  /usr/sbin/loadkeys /usr/share/keymaps/i386/${LAYOUT:-qwertz}/${KEYMAP:-de}.map.gz
  # Workaround for DVA1622
  if [ "${MODEL}" = "DVA1622" ]; then
    echo >/dev/tty2
    /usr/sbin/ioctl /dev/tty0 22022 -v 2
  ...
  echo "ExecStart=/usr/sbin/loadkeys /usr/share/keymaps/i386/${LAYOUT:-qwertz}/${KEYMAP:-de}.map.gz" >>${DEST}
  echo                                                                                              >>${DEST}
  echo "[Install]"                                                                                  >>${DEST}
